...
//----------------------------
{
  ...
  switch (ProcState)
  {
    case PC_WAIT_START: if (c == START_CHAR)      {
        PC_Mode = PCM_NORMAL;
        ProcState = PC_READ_TYP;
      }
    ...
    case PC_READ_TYP:   ProcState = Find_Activation_Character_in_Display_Array(c);
      ...
      {
        if (ProcState_Outside_Disp(2)) {
          ProcState = PC_ERROR;  // Savety check
          return;
        }
        ProcChrPtr = Disp_Txt[ProcState];
        memset(ProcChrPtr, 0, PB(&Disp[ProcState].size));
